...
- SCRIPT_DIR & APPROVED_PATHS<br>
    This will make the agent will give limit area for agent to only access to this dirrectory only. If we try put "cat /etc/passwd", the result will be error.
    ```
    SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
    ```

- is_path_within_allowed()<br>
    This will ensures the requested file is actually within an allowed directory.
    ```
    common = os.path.commonpath([SCRIPT_DIR, abs_requested])
    is_internal = common == SCRIPT_DIR
    ```

- check_and_approve()<br>
    If AI wants to access files outside the secure area, the user must grant permission first.
    ```
    response = input(
    f"Warning: Accessing external file {requested_path}. Allow? (y/n): "
    )
    ```

- Security check in main loop<br>
    This code performs fast filtering of sensitive locations.
    ```
    dangerous_patterns = ['/etc/', '~/', '$HOME', '/root/', '/home/']
    ```

- Print security scope on startup<br>
    THis will make all commands generated by AI must pass a security check first.
    ```
    if not check_command_security(command):
