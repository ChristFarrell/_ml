# reports/formatter.py — Format AnalysisReport to plain text or JSON (V1/V2/V3)
...
def format_plain(report: AnalysisReport) -> str:
    ...
    if report.intel:
        ...
        lines.append(f"  🕐 Domain age : {age_str}" + (" ← VERY NEW" if intel.is_young_domain else ""))
    ...
    lines += [
        ...
        f"  {r.recommendation}",
        "",
        f"  Analyzed: {r.created_at[:19].replace('T', ' ')}",
        sep, "",
    ]
    return "\n".join(lines)
